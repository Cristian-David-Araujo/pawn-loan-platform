...
- **`Loan.due_day` doubles as grace days** in the payments module: a charge's due date is `period_end + due_day` days. Past that, penalty = `pending * late_penalty_rate / 100`.
...
- **i18n is mandatory**: all UI strings go through `vue-i18n` keys defined in [messages.ts](apps/web-client/src/i18n/messages.ts) — every key must be added to **both** the `en` and `es` blocks. Locale persists to `localStorage`.
