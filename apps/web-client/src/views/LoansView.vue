      ...
      <table>
        ...
        <tbody>
          <tr v-for="loan in paginatedLoans" :key="loan.id" class="clickable-row" @click="openLoanDetail(loan.id)">
            ...
            <td class="text-right" :data-label="t('common.interest')">
            ...
            <td :data-label="t('common.createdBy')">{{ userLabel(loan.created_by) ?? '-' }}</td>
            <td>
...
const statusFilterOptions = computed(() => [
  ...
  /* Not a status — a paused loan is still `active` or `overdue`. It shares the control
     because the operator's question is the same one ("narrow this list"), and a pause was
     otherwise findable only by opening loans one at a time to see which button they offered. */
  { value: 'paused', label: t('loans.interestPaused') }
])
const search = ref('')
...
const STATUS_FILTERS = ['all', 'active', 'overdue', 'closed', 'defaulted', 'paused'] as const
...
const filteredLoans = computed(() => {
  ...
  const filtered = state.loans.filter((loan) => {
    const statusMatches =
      statusFilter.value === 'all' ||
      (statusFilter.value === 'paused' ? loan.interestPaused : loan.status === statusFilter.value)
