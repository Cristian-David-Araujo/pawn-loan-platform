  ...
  >
    <div :class="asPage ? 'loan-detail-shell' : 'modal-panel card modal-panel-lg'">
      ...
      <div class="stats-inline mt-16">
        <span class="pill">{{ t('common.status') }}: {{ t(loanStatusKey(loan.status)) }}</span>
        <span class="pill pill-paused" v-if="loan.interestPaused">
          <Pause :size="11" aria-hidden="true" />
          {{ t('loans.interestPaused') }}
        </span>
        ...
        <span class="pill">{{ t('common.createdBy') }}: {{ userLabel(loan.created_by) ?? '-' }}</span>
      </div>

      <p class="notice notice-info mt-16" v-if="loan.interestPaused">
        <strong>{{ t('loans.interestPaused') }}</strong><template v-if="loan.interestPauseReason">: {{ loan.interestPauseReason }}</template>
        · {{ t('loans.pauseKeepsExistingDebt') }}
      </p>
                ...
                <tr v-for="payment in paginatedLoanPayments" :key="payment.id">
                  ...
                  <td class="muted" :data-label="t('payments.notes')">
                    ...
                    <div v-if="payment.isReversed" class="pill pill-overdue mt-1">
                      ...
                      <template v-if="userLabel(payment.reverser)">
...
import { loanStatusKey } from '../utils/loanStatus'
