...
- `.github/actions/deploy-to-droplet/action.yml`
- `.github/scripts/calculate_version.sh`
- `deploy/digitalocean/production.env`
- `deploy/digitalocean/remote_deploy.sh`
...
- Nothing else: `.env.production` is generated by the deploy, not prepared by hand.
...
There is no `DO_APP_DIR` secret. The deploy path lives in the `app_dir` input of the
[deploy action](../.github/actions/deploy-to-droplet/action.yml), defaulting to
`/opt/pawn-loan-platform`. It is versioned rather than secret on purpose: a path is not
sensitive, and while it was a secret a wrong value failed a release with
`cd: ***: No such file or directory` — the masking hid the one thing the error needed to say.
...
```bash
cd /opt/pawn-loan-platform            # the app_dir input of the deploy action
git fetch --all --tags --prune
git checkout main
...
2. Confirm the project is cloned at the deploy path (`/opt/pawn-loan-platform`).
...
- Add `DO_HOST`, `DO_USER`, `DO_SSH_KEY_B64` (or `DO_SSH_KEY`), `POSTGRES_PASSWORD`,
  `JWT_SECRET_KEY` and `ADMIN_PASSWORD`.
...
- Verify exact secret names (`DO_HOST`, `DO_USER`, `DO_SSH_KEY_B64` or `DO_SSH_KEY`,
  `POSTGRES_PASSWORD`, `JWT_SECRET_KEY`, `ADMIN_PASSWORD`).
- Setting a secret from Git Bash on Windows mangles a value that starts with `/`: MSYS rewrites
  it into a Windows path before `gh` ever sees it. Prefix the command with `MSYS_NO_PATHCONV=1`.
...
3. Error: `cd: ...: No such file or directory` on the droplet
- The deploy path is the `app_dir` input of the deploy action, defaulting to
  `/opt/pawn-loan-platform`. Confirm the repository is cloned there.
...
- Pass `app_dir` to the deploy action in both workflows.
